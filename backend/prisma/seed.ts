...
async function main() {
  ...
  const admin = await prisma.user.create({
    data: { email: 'admin@enjay.com', passwordHash: defaultPassword, fullName: 'System Admin', avatar: 'https://i.pravatar.cc/150?u=admin', roleId: roleAdmin.id, isActive: true, isEmailVerified: true }
  });

  const manager = await prisma.user.create({
    data: { email: 'manager@enjay.com', passwordHash: passCommon, fullName: 'Support Manager', avatar: 'https://i.pravatar.cc/150?u=manager', roleId: roleManager.id, isActive: true, isEmailVerified: true }
  });

  const eng1 = await prisma.user.create({
    data: { email: 'engineer1@enjay.com', passwordHash: passCommon, fullName: 'Jane Engineer', avatar: 'https://i.pravatar.cc/150?u=eng1', roleId: roleEngineer.id, isActive: true, isEmailVerified: true }
  });

  const eng2 = await prisma.user.create({
    data: { email: 'engineer2@enjay.com', passwordHash: passCommon, fullName: 'Bob Engineer', avatar: 'https://i.pravatar.cc/150?u=eng2', roleId: roleEngineer.id, isActive: true, isEmailVerified: true }
  });

  const cust1 = await prisma.user.create({
    data: { email: 'customer1@enjay.com', passwordHash: passCommon, fullName: 'Alice Customer', avatar: 'https://i.pravatar.cc/150?u=cust1', roleId: roleCustomer.id, isActive: true, isEmailVerified: true }
  });
  ...
  await prisma.ticketSequence.create({
    data: { id: "TICKET_SEQ", lastValue: seq - 1 }
  });
